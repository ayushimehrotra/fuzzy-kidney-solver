...
def expected_utility(cycle, digraph, edge_success_prob=0.7, vertex_success_prob=0.7):
    """
    calculates the expected utility of a cycle

    Arguments
    cycle: the cycle in the graph
    digraph: a Digraph object
    edge_success_prob: the probability of success of an edge
    vertex_success_prob: the probability of success of a vertex
    """
...
def choice_utility(cycle, digraph, edge_success_prob=0.7, vertex_success_prob=0.7, donor=0.1, patient=0.1):
    """
        calculates the choice expected utility of a cycle
        ...
        patient: lambda
        """
    return sum(digraph.adj_mat[cycle[i - 1].id][cycle[i].id].donor_score * donor + digraph.adj_mat[cycle[i - 1].id][
        cycle[i].id].patient_score * patient
               for i in range(len(cycle))) * edge_success_prob ** len(cycle) * vertex_success_prob ** (len(cycle))


def weighted_choice_utility(cycle, digraph, edge_success_prob=0.7, vertex_success_prob=0.7, donor=0.1, patient=0.1):
    """
        calculates the weighted choice expected utility of a cycle
        ...
        """
    return sum(digraph.adj_mat[cycle[i - 1].id][cycle[i].id].donor_score * donor + digraph.adj_mat[cycle[i - 1].id][
        cycle[i].id].patient_score * patient + digraph.adj_mat[cycle[i - 1].id][cycle[i].id].score
               for i in range(len(cycle))) * edge_success_prob ** len(cycle) * vertex_success_prob ** (len(cycle))
...
class Edge:
    ...
    def __init__(self, id, score, donor_score, patient_score, src, tgt):
        self.id = id
        self.score = score
...
class Digraph:
    """A directed graph, in which each edge has a numeric score.
        ...
        Data members:
            ...
            ces: an array of Choice objects
            adj_mat = adjacency matrix for edges
            adj_mat_choice: adjancency matrix for choice edges
    """
    ...
    def add_edge(self, score, donor_score, patient_score, source, tgt):
        """Add an edge to the digraph

        Args:
            score: the edge's score, as a float
            patient_score: the attitude of the patient towards the donor
            donor_score: the attitude of the donor towards the patient
            source: the source Vertex
            tgt: the edge's target Vertex
        """

        id = len(self.es)
        e = Edge(id, score, donor_score, patient_score, source, tgt)
        self.es.append(e)
        ...
        for i, v in enumerate(vertices):
            for j, w in enumerate(vertices):
                ...
                if e is not None:
                    ...
                    subgraph.add_edge(e.score, e.donor_score, e.patient_score, new_src, new_tgt)
        return subgraph
...
def read_digraph(lines):
    ...
    for line in lines[1:edge_count + 1]:
        ...
        digraph.add_edge(score, donor_score, patient_score, digraph.vs[src_id], digraph.vs[tgt_id])
